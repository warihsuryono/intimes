        ...
        <div class="row">
            <div class="col-lg-12">
                                ...
                                <form method="GET" id="filter_form">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-3">
                                            <div class="form-group">
                                                <label>Scope</label>
                                                <input name="name" value="<?= @$_GET["name"]; ?>" type="text" class="form-control" placeholder="Item Scope ...">
                                            </div>
                                        </div>
                                        <div class="col-sm-3">
                                            <div class="form-group">
                                                ...
                                                <select name="unit_id" class="form-control">
                                                    ...
                                                    <?php foreach ($units as $unit) : ?>
                                                        <option value="<?= $unit->id; ?>" <?= (@$_GET["unit_id"] == $unit->id) ? "selected" : ""; ?>><?= $unit->name; ?></option>
                                                    <?php endforeach ?>
                                                </select>
                                            </div>
                                        </div>
                ...
                <div class="row" style="margin-bottom:10px;">
                    <div class="col-2">
                        <button class="btn btn-primary" onclick="window.location='<?= base_url(); ?>/item_scope/add';"><i class="fa fa-plus"></i></button>
                    </div>
                </div>
                <div class="card">
                    <!--PAGING -->
                    ...
                    <div class="card-body table-responsive p-0" style="height: 700px;">
                        <table class="table table-head-fixed text-nowrap table-striped">
                            ...
                            <tbody>
                                <?php
                                $no = $startrow;
                                foreach ($item_scopes as $i_scope) :
                                    $no++;
                                ?>
                                    <tr>
                                        <td>
                                            <a class="btn btn-info btn-sm" href="<?= base_url(); ?>/item_scope/edit/<?= $i_scope->id; ?>">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a class="btn btn-danger btn-sm" href="#" onclick="delete_confirmation(<?= $i_scope->id; ?>);">
                                                <i class="fas fa-trash"></i>
                                            </a>
                                        </td>
                                        <td><?= $no; ?></td>
                                        <td><?= $i_scope->id; ?></td>
                                        <td><?= $i_scope_detail[$i_scope->id]["item_type"]; ?></td>
                                        <td><?= $i_scope->name; ?></td>
                                        <td><?= $i_scope_detail[$i_scope->id]["unit"]; ?></td>
                                        <td align="right"><?= $i_scope->add_price; ?></td>
                                        <td><?= $i_scope->price_currency_id; ?></td>
                                        <td><?= date("d-m-Y H:i:s", strtotime($i_scope->created_at)); ?></td>
                                        <td><?= $i_scope->created_by; ?></td>
                                    </tr>
                                <?php endforeach ?>
                            </tbody>
                        </table>
