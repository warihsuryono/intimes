        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="row" style="margin-bottom:10px;">
                    <div class="col-2">
                        <button class="btn btn-primary" onclick="window.location='<?= base_url(); ?>/tire_type/add';"><i class="fa fa-plus"></i></button>
                    </div>
                </div>
                <div class="card">
                    <!--PAGING -->
                    ...
                    <div class="card-body table-responsive p-0" style="height: 700px;">
                        <table class="table table-head-fixed text-nowrap table-striped">
                            ...
                            <tbody>
                                <?php
                                $no = $startrow;
                                foreach ($tire_types as $tire_type) :
                                    $no++;
                                ?>
                                    <tr>
                                        <td>
                                            <a class="btn btn-info btn-sm" href="<?= base_url(); ?>/tire_type/edit/<?= $tire_type->id; ?>">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a class="btn btn-danger btn-sm" href="#" onclick="delete_confirmation(<?= $tire_type->id; ?>);">
                                                <i class="fas fa-trash"></i>
                                            </a>
                                        </td>
                                        <td><?= $no; ?></td>
                                        <td><?= $tire_type->id; ?></td>
                                        <td><?= $tire_type->name; ?></td>
                                        <td><?= date("d-m-Y H:i:s", strtotime($tire_type->created_at)); ?></td>
                                        <td><?= $tire_type->created_by; ?></td>
                                    </tr>
                                <?php endforeach ?>
                            </tbody>
                        </table>
