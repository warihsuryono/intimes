        ...
        <div class="row">
            <div class="col-lg-12">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-3">
                                            <div class="form-group">
                                                ...
                                                <input name="name" value="<?= @$_GET["name"]; ?>" type="text" class="form-control" placeholder="Item Specification ...">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row">
                ...
                <div class="row" style="margin-bottom:10px;">
                    <div class="col-2">
                        <button class="btn btn-primary" onclick="window.location='<?= base_url(); ?>/item_specification/add';"><i class="fa fa-plus"></i></button>
                    </div>
                </div>
                <div class="card">
                    <!--PAGING -->
                    ...
                    <div class="card-body table-responsive p-0" style="height: 700px;">
                        <table class="table table-head-fixed text-nowrap table-striped">
                            ...
                            <tbody>
                                <?php
                                $no = $startrow;
                                foreach ($item_specifications as $i_specification) :
                                    $no++;
                                ?>
                                    <tr>
                                        <td>
                                            <a class="btn btn-info btn-sm" href="<?= base_url(); ?>/item_specification/edit/<?= $i_specification->id; ?>">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a class="btn btn-danger btn-sm" href="#" onclick="delete_confirmation(<?= $i_specification->id; ?>);">
                                                <i class="fas fa-trash"></i>
                                            </a>
                                        </td>
                                        <td><?= $no; ?></td>
                                        <td><?= $i_specification->id; ?></td>
                                        <td><?= $i_specification->name; ?></td>
                                        <td><?= date("d-m-Y H:i:s", strtotime($i_specification->created_at)); ?></td>
                                        <td><?= $i_specification->created_by; ?></td>
                                    </tr>
                                <?php endforeach ?>
                            </tbody>
                        </table>
...
<script>
    function delete_confirmation(id) {
        $('#modal_title').html('Delete Item Specifications');
        $('#modal_message').html('Are you sure want to delete this data?');
        $('#modal_type').attr("class", 'modal-content bg-danger');
        $('#modal_ok_link').attr("href", '<?= base_url(); ?>/item_specification/delete/' + id);
        $('#modal-form').modal();
    }
</script>