        ...
        <div class="row">
            <div class="col-lg-12">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-3">
                                            <div class="form-group">
                                                ...
                                                <input name="name" value="<?= @$_GET["name"]; ?>" type="text" class="form-control" placeholder="Item Sub Category ...">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row">
                ...
                <div class="row" style="margin-bottom:10px;">
                    <div class="col-2">
                        <button class="btn btn-primary" onclick="window.location='<?= base_url(); ?>/item_sub_category/add';"><i class="fa fa-plus"></i></button>
                    </div>
                </div>
                <div class="card">
                    <!--PAGING -->
                    ...
                    <div class="card-body table-responsive p-0" style="height: 700px;">
                        <table class="table table-head-fixed text-nowrap table-striped">
                            ...
                            <tbody>
                                <?php
                                $no = $startrow;
                                foreach ($item_sub_categories as $i_sub_category) :
                                    $no++;
                                ?>
                                    <tr>
                                        <td>
                                            <a class="btn btn-info btn-sm" href="<?= base_url(); ?>/item_sub_category/edit/<?= $i_sub_category->id; ?>">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a class="btn btn-danger btn-sm" href="#" onclick="delete_confirmation(<?= $i_sub_category->id; ?>);">
                                                <i class="fas fa-trash"></i>
                                            </a>
                                        </td>
                                        <td><?= $no; ?></td>
                                        <td><?= $i_sub_category->id; ?></td>
                                        <td><?= $item_sub_category_detail[$i_sub_category->id]["item_category"]; ?></td>
                                        <td><?= $i_sub_category->name; ?></td>
                                        <td><?= date("d-m-Y H:i:s", strtotime($i_sub_category->created_at)); ?></td>
                                        <td><?= $i_sub_category->created_by; ?></td>
                                    </tr>
                                <?php endforeach ?>
                            </tbody>
                        </table>
...
<script>
    function delete_confirmation(id) {
        $('#modal_title').html('Delete Item Sub Categories');
        $('#modal_message').html('Are you sure want to delete this data?');
        $('#modal_type').attr("class", 'modal-content bg-danger');
        $('#modal_ok_link').attr("href", '<?= base_url(); ?>/item_sub_category/delete/' + id);
        $('#modal-form').modal();
    }
</script>